...
tournament_dates = golf['Date'].dropna().unique()
...
for date in tournament_dates_list:
	...
	if len(round) == 18:
		...
		for num in range (0, len(par45_df)):
			...
			if second_shot_condition == 'Fairway':
	...
	if len(round) == 18:
		for num in range(0, 18):
			...
			for c in range(0,9):
				...
				unit = unit_list[c]
				print(hole[tohole])
				if int(hole[tohole]) <= 50:
					if hole[unit] == 'Yard':
						distance_around = hole[tohole]
						...
						tohole3 = 'To_Hole' + y
						club2 = 'Club_' + x
						nextdistance = hole[tohole2]
						distance3 = hole[tohole3]
